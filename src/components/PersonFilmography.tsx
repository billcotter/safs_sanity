...
export function PersonFilmography({
  ...
  // Helper function to create film slug from title
  const createFilmSlug = (title: string) => {
    return title
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
      ...
      <Card className='p-6 bg-gradient-to-r from-ocean-blue/5 to-sandstone/20 border-ocean-blue/30'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
          {knownFor.slice(0, 6).map((movie: any, index: number) => (
            ...
            >
              <Link href={`/films/${createFilmSlug(movie.title)}`}>
                <div className='aspect-[2/3] bg-gradient-to-br from-ocean-blue/10 to-terracotta/10 rounded-lg mb-3 flex items-center justify-center hover:scale-105 transition-transform cursor-pointer'>
                  {movie.poster_path ? (
                    <img
              ...
              <h4 className='font-semibold text-charcoal text-sm mb-1 line-clamp-2'>
                <Link
                  href={`/films/${createFilmSlug(movie.title)}`}
